...
class MainWindow(QMainWindow):
    ...
    def on_cell_clicked(self, row, column):
        ...
        # 结果判断
        if any(item in self.cls_list for item in [0,2,3,4,5]):
            res = '警告'
            self.ui.label_res.setStyleSheet("background-color: rgb(255, 0, 0);")
        else:
            res = '正常'
            self.ui.label_res.setStyleSheet("background-color: rgb(0, 255, 0);")
        self.ui.label_res.setText(res)
    ...
    def set_alarm_action(self):
        ...
        if any(item in self.cls_list for item in [0,2,3,4,5]):
            ...
            if not self.is_alarm_on:
                self.alarm_thread = Thread(target=self.sound_alarm, args=(Config.alarm_path,))
                self.alarm_thread.deamon = True
